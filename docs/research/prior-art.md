...
Python project (v0.9.x, active, small user base) with PoB vendored as a git submodule. Architecture: a Python-to-Lua bridge with a persistent worker pool that keeps Lua state alive across evaluations (about 2x speedup from batching, parallel across cores). Scope is deliberately narrow:
...
- Does not touch items, gems, ascendancy or keystones; cannot start from scratch; existing builds only.
...
- pobarchives.com: about 7300 curated builds with leaguestarter/trending/author filters; this repo already ships a provider (`src/Classes/PoBArchivesProvider.lua`).
- pobapi (Python) parses PoB share codes if a non-Lua consumer is ever needed.
- GGG official ladder/character/public-stash APIs are the sanctioned bulk data route.
